...
setInterval(() => {
  ...
  emitNet('np-weathersync:client:weather', -1, currentWeather)
}, timePerWeather * 1000)

onNet('np-weathersync:client:weather:request', () => {
  emitNet('np-weathersync:client:weather', global.source, weatherProgression[0])
})

RegisterCommand('weather', (source: string, args: string[]) => {
  if (!IsPlayerAceAllowed(source, "np-weathersync.commands.weather")) {
  ...
  emitNet('np-weathersync:client:weather', -1, weatherProgression[0])
}, false)
